...
- **Conversation compaction** (`agent.py`, `compact` graph node): `thread_id == conversation_id`, so each Teams conversation is ONE persistent checkpoint the saver appends to forever — `trim_messages` only trims the in-flight copy, never the stored state, so threads grew to thousands of messages (~2.5M est tokens) reloaded/re-trimmed every turn. The terminal `compact` node summarizes the oldest turns into a rolling `summary` (persisted in `AgentState`, injected into the system prompt) and `RemoveMessage`s them, keeping the checkpoint bounded. Runs on the terminal hop so its latency is off the reply's reasoning path and its update never clobbers the streamed reply. The kept window always starts on a `HumanMessage` so tool-call/result pairs are never split (Gemini 400s on orphaned function parts). Thresholds are chosen so a compacted thread stays BELOW `_MAX_INPUT_TOKENS` (300k) — the stored window oscillates between 150k and 200k, so `trim_messages` never has to silently truncate a compacted thread and the model sees the whole stored history plus the summary. That is what makes compaction accuracy-neutral: it replaces *silent* lossy truncation at 300k (the old behavior on runaway threads) with a summarized, fully-visible window. The rolling summary is hard-capped (`_cap_summary`) as well as prompt-instructed, because it is re-injected into every later turn and fed back into the next compaction — uncapped, it would ratchet every prompt upward forever. Kill switch + tuning: `SAMURAI_COMPACT_MODE` (default `on`, `off` to disable), `SAMURAI_COMPACT_TRIGGER_TOKENS` (200k), `SAMURAI_COMPACT_KEEP_TOKENS` (150k), `SAMURAI_COMPACT_TRIGGER_MSGS` (400 — only forces the cut to be *evaluated*; the cut itself is governed by keep-tokens), `SAMURAI_COMPACT_SUMMARY_MAX_CHARS` (6000), `SAMURAI_COMPACT_MAX_SUMMARIZE_TOKENS` (60k). **The per-pass summarization payload is bounded** (`_select_summarizable_tail`): steady-state compaction only drops the trigger→keep delta (~50k) and summarizes all of it, but the FIRST compaction of an already-runaway thread drops everything older than the keep window — on the logged 2.5M-token thread that was a ~2M-token single call, past the model's context window, so it raised, the error was swallowed, and the thread never compacted (silent no-op on exactly the threads the feature exists for). Now the newest slice within budget is summarized and anything older is removed unsummarized — not a context regression, since messages beyond `_MAX_INPUT_TOKENS` were already invisible to the model — so catch-up converges in one pass. Watch `discarded_unsummarized=` in the `[compact]` log line: non-zero means a catch-up pass; it should be 0 in steady state. To hard-reset a runaway thread, `memory.clear_thread(conversation_id)` still wipes it entirely.
- **Heavy background pipelines are serialized by a NON-BLOCKING lock** (`scheduler._BG_PIPELINE_LOCK` via `_run_under_pipeline_lock`): the KB compile, engineering sync, self-tune cycle, and tracker triage all call Gemini, so at most one runs at a time (quota protection). A pipeline that fires while another holds the lock **defers** (returns immediately) instead of blocking on it — blocking kept the job alive as an APScheduler "running instance" and, when its own cron re-fired, tripped `max_instances=1` into `"skipped: maximum number of running instances reached"` log storms. `coalesce=True` + the next scheduled fire re-run a deferred tick; the KB compile is checkpoint-as-you-go so nothing is lost. During the initial full KB build (`KB_PIPELINE_CRON=*/5`), deferrals are expected and benign — relax the cron to daily once built.
